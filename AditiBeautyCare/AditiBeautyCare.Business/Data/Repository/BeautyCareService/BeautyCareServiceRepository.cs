...
namespace AditiBeautyCare.Business.Data.Repository.BeautyCareService
{
    public class BeautyCareRepository:IBeautyCareServiceRepository
    {
        ...
        {
            using IDbConnection db = _connectionFactory.GetConnection;
            return db.Query<BeautyCareServiceModel>("Select top 1 * From [BeautyCareServices] where ServiceId=@id and IsDeleted=0", new { id }).FirstOrDefault();
        }

        public IPagedList<BeautyCareServiceModel> GetPages(int pageIndex = 1, int pageSize = 10)
        {
            using IDbConnection db = _connectionFactory.GetConnection;
            var query = db.QueryMultiple("SELECT COUNT(*) FROM [BeautyCareServices] where IsDeleted=0;SELECT* FROM [BeautyCareServices] where IsDeleted=0 ORDER BY ServiceId desc OFFSET ((@PageNumber - 1) * @Rows) ROWS FETCH NEXT @Rows ROWS ONLY", new { PageNumber = pageIndex, Rows = pageSize }, commandType: CommandType.Text);
